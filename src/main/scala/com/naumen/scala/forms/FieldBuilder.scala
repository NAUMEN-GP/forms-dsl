...
case class FormDescriptionBuilder[+T: Manifest](fields: Map[String, FieldDescription] = Map(), attrs: Map[String, Any] = Map()) extends FieldNameGetter {
    ...
    type FormField[F, +U] = FieldFoo[F] => FormDescriptionBuilder[U]
    ...
    def boolean(fieldFoo: T => Boolean): FormField[Boolean, T] = fieldBase[Boolean](fieldFoo)(_.required)

    def string(fieldFoo: T => String): FormField[String, T] =
        fieldBase[String](fieldFoo)(_.required)

    def stringOpt(fieldFoo: T => Option[String]): FormField[String, T] =
        fieldBase[String](fieldFoo)(identity)

    def int(fieldFoo: T => Int): FormField[Int, T] = fieldBase[Int](fieldFoo)(_.required)

    def intOpt(fieldFoo: T => Option[Int]): FormField[Int, T] = fieldBase[Int](fieldFoo)(identity)

    def bigDecimal(fieldFoo: T => BigDecimal): FormField[BigDecimal, T] = fieldBase[BigDecimal](fieldFoo)(_.required)

    def bigDecimalOpt(fieldFoo: T => Option[BigDecimal]): FormField[BigDecimal, T] = fieldBase[BigDecimal](fieldFoo)(identity)
    ...
    def dateOpt(fieldFoo: T => Option[Date]): FormField[Date, T] = fieldBase[Date](fieldFoo)(_.format(defaultDateFormat))

    def date(fieldFoo: T => Date): FormField[Date, T] = fieldBase[Date](fieldFoo)(_.required.format(defaultDateFormat))
